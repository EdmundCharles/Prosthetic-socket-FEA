...
def run_ccx(job_name="job"):
    ...
    ccx_path_mac = "/opt/homebrew/bin/ccx_2.23"
    ccx_path_win = "C:\\Calculix\\bin\\ccx_2.23"
    process = subprocess.run([ccx_path_win, "-i", job_name], env=my_env, capture_output=True, text=True)
...
def process_socket_analysis(stl_path, load_newtons, condition="II", mesh_size=5.0, search_depth=150.0, material="petg_ortho"):
    ...
    # ВОЗВРАЩАЕМ ТОЛЬКО СТАТИКУ (БЕЗ УСТАЛОСТИ)
    return {
        "nodes": nodes.tolist(), 
